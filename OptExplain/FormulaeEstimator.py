import numpy as np
from OptExplain.silas import add_f, sub_f
    ...
    def get_formulae_text(self, file):
        ...
        if self._conjunction is True:
            ...
            for index, formula in enumerate(self.groups_formula):
                ...
                groupArr.append({'Groups': 'Group {}'.format(index),'samples':sum(self.group_weights[index]), 'rules':len(self._groups[index][1]),'Class':self._groups[index][0], 'Rules':text })
                file.write(title + '\n')
                file.write(text + '\n')
            # print('gggg',groupArr)
        else:
