...
from OptExplain.Extractor import Extractor
from OptExplain.Z3Process import Z3Process
from OptExplain.FormulaeEstimator import FormulaeEstimator
...
class MainProcess(object):
    ...
    def explain(self, param, label='', auc_plot=False):
        ...
        formulae_text_list = f.get_formulae_text(self._file)
        ...
        performance_ = {
            'Sample size': len(self._y_test),
            'RF accuracy': "{:.3f}".format(RF_accuracy),
            'RF AUC': "{:.3f}".format(ori_auc),
            'EX accuracy':"{:.3f}".format(EX_accuracy),
            'EX AUC': "{:.3f}".format(ex_auc),
            'Coverage': (len(self._y_test) - no_ans) / len(self._y_test),
            'Overlap':  "{:.3f}".format((overlap / len(self._y_test))),
            'Performance': "{:.3f}".format(performance),
        }
        ...
        if auc_plot is True:
           ...
           plt.legend(loc=4)
           plt.show()
        return formulae_text_list, performance_
